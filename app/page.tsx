...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    async function fetchAdditionalData() {
      try {
        if (isDevelopment) {
          ...
          const mockExpenses = [
            { id: '1', title: 'Office Supplies', amount: 1500, category: 'Office', date: '2024-01-15', opportunity_id: '1', description: 'Monthly office supplies', created_at: '2024-01-15T10:00:00Z', updated_at: '2024-01-15T10:00:00Z' },
            { id: '2', title: 'Software License', amount: 299, category: 'Software', date: '2024-01-10', opportunity_id: '2', description: 'Development tools subscription', created_at: '2024-01-10T10:00:00Z', updated_at: '2024-01-10T10:00:00Z' }
          ];
          const mockContacts = [
            { id: '1', first_name: 'Jane', last_name: 'Smith', email: 'jane@techcorp.com', phone: '+1234567890', position: 'CEO', company_id: '1', avatar_url: undefined, created_at: '2024-01-15T10:00:00Z', updated_at: '2024-01-15T10:00:00Z', company: { id: '1', name: 'TechCorp Inc' } },
            { id: '2', first_name: 'Mike', last_name: 'Johnson', email: 'mike@shopflow.com', phone: '+1234567891', position: 'CTO', company_id: '2', avatar_url: undefined, created_at: '2024-01-10T10:00:00Z', updated_at: '2024-01-10T10:00:00Z', company: { id: '2', name: 'ShopFlow Ltd' } }
          ];
          const mockCompanies = [
            { id: '1', name: 'TechCorp Inc', industry: 'Technology', website: 'https://techcorp.com', phone: '+1234567890', address: '123 Tech St', city: 'San Francisco', country: 'USA', logo_url: undefined, created_at: '2024-01-15T10:00:00Z', updated_at: '2024-01-15T10:00:00Z' },
            { id: '2', name: 'ShopFlow Ltd', industry: 'E-commerce', website: 'https://shopflow.com', phone: '+1234567891', address: '456 Commerce Ave', city: 'New York', country: 'USA', logo_url: undefined, created_at: '2024-01-10T10:00:00Z', updated_at: '2024-01-10T10:00:00Z' }
          ];
    ...
    // Set up real-time subscriptions (skip in development mode)
    let expensesSubscription: any = null;
    let contactsSubscription: any = null;
    let companiesSubscription: any = null;
  ...
  const metrics = useMemo(() => {
    ...
    const wonOpportunities = opportunitiesList.filter(o => o.status === 'won').length;
    const lostOpportunities = opportunitiesList.filter(o => o.status === 'lost').length;
    const qualifiedOpportunities = opportunitiesList.filter(o => ['qualified', 'proposal', 'negotiation'].includes(o.status)).length;
    
    const totalRevenue = opportunitiesList.filter(o => o.status === 'won').reduce((sum, o) => sum + Number(o.amount || 0), 0);
    const forecastRevenue = opportunitiesList.filter(o => !['won', 'lost'].includes(o.status)).reduce((sum, o) => sum + Number(o.forecast_amount || o.amount || 0), 0);
  ...
  const pipelineData = useMemo(() => [
    { 
      name: 'Leads', 
      value: opportunities.filter(o => o.status === 'lead').length,
      color: '#6366f1',
      amount: opportunities.filter(o => o.status === 'lead').reduce((sum, o) => sum + Number(o.amount), 0)
    },
    { 
      name: 'Qualified', 
      value: opportunities.filter(o => o.status === 'qualified').length,
      color: '#3b82f6',
      amount: opportunities.filter(o => o.status === 'qualified').reduce((sum, o) => sum + Number(o.amount), 0)
    },
    { 
      name: 'Proposal', 
      value: opportunities.filter(o => o.status === 'proposal').length,
      color: '#06b6d4',
      amount: opportunities.filter(o => o.status === 'proposal').reduce((sum, o) => sum + Number(o.amount), 0)
    },
    { 
      name: 'Negotiation', 
      value: opportunities.filter(o => o.status === 'negotiation').length,
      color: '#10b981',
      amount: opportunities.filter(o => o.status === 'negotiation').reduce((sum, o) => sum + Number(o.amount), 0)
    },
    { 
      name: 'Won', 
  ...
  const revenueTrendData = useMemo(() => {
    ...
    return months.map((month, index) => ({
      month,
      revenue: Math.max(0, baseRevenue * (0.8 + Math.random() * 0.4)),
      target: Math.max(0, baseRevenue * (0.9 + Math.random() * 0.2)),
      actual: index < 5 ? Math.max(0, baseRevenue * (0.7 + Math.random() * 0.3)) : baseRevenue
    }));
  ...
  const sectorData = useMemo(() => {
    const sectors = opportunities.reduce((acc: any[], opp) => {
      ...
      if (existing) {
        ...
        existing.count += 1;
        existing.won += opp.status === 'won' ? 1 : 0;
      } else {
        acc.push({ name: sector, value: Number(opp.amount), count: 1, won: opp.status === 'won' ? 1 : 0 });
      }
      return acc;
    }, []);
  ...
  // Show dashboard immediately after authentication - data loads in background
