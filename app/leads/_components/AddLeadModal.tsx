...
export default function AddLeadModal({ open, onClose, leadToEdit }: AddLeadModalProps) {
  ...
  useEffect(() => {
    if (leadToEdit) {
      setForm({
        ...
        opportunity_type: "Manual",
        lead_status: leadToEdit.lead_status || "Cold",
        forecast: leadToEdit.forecast || "",
        description: leadToEdit.description || "",
        tat: leadToEdit.tat || "",
        remarks: leadToEdit.remarks || "",
        selectedCompanyId: leadToEdit.company_id || "",
        company_name: leadToEdit.company?.name || "",
        company_website: leadToEdit.company?.website || "",
        company_email: leadToEdit.company?.email || "",
        company_phone: leadToEdit.company?.phone || "",
        company_sector: "",
        poc_name: leadToEdit.company?.poc?.name || "",
        poc_email: leadToEdit.company?.poc?.email || "",
        poc_phone: leadToEdit.company?.poc?.phone || "",
        poc_importance: leadToEdit.company?.poc?.importance || "Medium",
        open_date: leadToEdit.open_date ? new Date(leadToEdit.open_date).toISOString().split('T')[0] : "",
        close_date: leadToEdit.close_date || "",
        competitors: leadToEdit.competitors || [],
      });
      setCompanyMode(leadToEdit.company_id ? 'select' : 'create');
    } else {
      setForm({
        title: "",
        opportunity_type: "Manual",
  ...
  const handleCompanySelect = (companyId: string) => {
    ...
    if (selectedCompany) {
      setForm(prev => ({
        ...
        poc_email: selectedCompany.poc?.email || "",
        poc_phone: selectedCompany.poc?.phone || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      tat: form.tat || undefined,
      remarks: form.remarks || undefined,
      lead_status: form.lead_status as 'Cold' | 'Warm' | 'Hot' | 'Won' | 'Lost',
      open_date: form.open_date || new Date().toISOString().split('T')[0],
      close_date: form.close_date || undefined,
      competitors: form.competitors.map(c => ({
        ...c,
        status: c.status as 'Winning' | 'Losing' | 'Equal'
      })),
    };
