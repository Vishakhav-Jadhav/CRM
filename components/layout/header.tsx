...
export function Header({ onMenuClick }: HeaderProps) {
  ...
  useEffect(() => {
    ...
    const highPriorityOpps = opportunities.filter(opp =>
      opp.priority === 'high' && opp.status !== 'won' && opp.status !== 'lost'
    );
            ...
            <div className="max-h-80 overflow-y-auto">
              {notificationCount === 0 ? (
                ...
                <div className="p-2">
                  ...
                  {opportunities.filter(o => o.priority === 'high' && o.status !== 'won' && o.status !== 'lost').slice(0, 3).map((opportunity) => (
          ...
          <PopoverTrigger asChild>
            <Button variant="ghost" className="relative h-8 w-8 sm:h-9 sm:w-9 p-0">
              <Avatar className="h-8 w-8 sm:h-9 sm:w-9">
                <AvatarImage src={user?.user_metadata?.avatar_url || settings?.user_avatar} />
