...
export default function LeadKanban({
  ...
  const grouped = columns.reduce((acc, col) => {
    acc[col.key] = leads.filter(l => l.lead_status === col.key);
    return acc;
  ...
  function handleDragOver(event: DragOverEvent) {
    ...
    if (activeContainer !== overContainer) {
      ...
      if (activeLead) {
        updateLead(activeId, { lead_status: overContainer as Lead['lead_status'] });
      }
    }
  }
  ...
  function findContainer(id: string) {
    ...
    return lead ? lead.lead_status : null;
  }
        ...
        {activeLead ? (
          <Card className="p-3 shadow-lg bg-background border-2 border-primary">
            <div className="flex items-start justify-between">
              <div>
                <div className="font-medium">{activeLead.title}</div>
                <div className="text-xs text-muted-foreground">{activeLead.company?.name || ""}</div>
                <div className="text-xs text-muted-foreground mt-1">{activeLead.forecast || ""}</div>
              </div>
              <div className="text-sm font-semibold">{activeLead.amount ? `₹${Number(activeLead.amount).toLocaleString()}` : ""}</div>
            </div>
          </Card>
        ) : null}
      </DragOverlay>
