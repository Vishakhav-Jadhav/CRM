...
export default function LeadFormModal({
  ...
  useEffect(() => {
    if (leadToEdit) {
      setForm({
        ...
        opportunity_type: "Manual", // or from leadToEdit if added
        company_id: leadToEdit.company_id || "",
        amount: leadToEdit.amount ? String(leadToEdit.amount) : "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      competitors: form.competitors.map(c => ({ ...c, status: c.status as 'Winning' | 'Losing' | 'Equal' })),
    };
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <Input
                  type="date"
                  value={leadToEdit?.open_date ? new Date(leadToEdit.open_date).toISOString().split('T')[0] : new Date().toISOString().split('T')[0]}
                  disabled
                />
              </div>
              <div>
